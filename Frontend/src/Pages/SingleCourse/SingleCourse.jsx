  ...
  useEffect(() => {
    const fetchDetails = async () => {
      const response = await fetch(
        `http://localhost:3014/paidcourse/${param.name}`
      );
  ...
  return (
    <>
      ...
      <div className="max-w-screen-2xl container mx-auto md:px-20 px-4 py-36 bg-slate-50 dark:bg-slate-900 dark:text-white">
        {product.map((item, id) => (
          <section className="text-gray-600 body-font overflow-hidden " key={id}>
            <div className="container px-5 mx-auto">
              <div className="lg:w-4/5 mx-auto flex flex-wrap">
                <img
                  alt="ecommerce"
                  className="md:w-[450px] w-full md:h-[400px] object-contain rounded"
                  src={item.image}
                />
                <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0 dark:text-white">
                  ...
                  <h1 className="text-gray-900 text-xl md:text-2xl title-font font-medium mb-1 dark:text-white">
                    {item.title}
                  </h1>
                  ...
                  <div className="flex mt-8 justify-between">
                    <span className="title-font font-medium text-2xl text-red-900 line-through">
                       ${item.price}
                    </span>
                     <a href="https://youtu.be/QFaFIcGhPoM?list=PLC3y8-rFHvwgg3vaYJgHGnModB54rxOk3" target="_blank">
